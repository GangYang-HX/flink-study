...
public class FileSourceSplitReader implements SplitReader<RecordAndPosition, FileSourceSplit> {
    ...
    @Nullable
    private FSDataInputStream currentInStream;
    @Nullable
    ...
    @Override
    public void handleSplitsChanges(SplitsChange<FileSourceSplit> splitsChange) {
    ...
    private void checkSplitOrStartNext() throws IOException {
        ...
        if (offset < 0 || offset > fileLength) {
            throw new IOException(String.format("Invalid offset for split %s ,file length is %d", nextSplit, fileLength));
        }

        currentInStream = fs.open(file);
        currentInStream.seek(offset);

        currentReader = new BufferedReader(new InputStreamReader(currentInStream, StandardCharsets.UTF_8));
        currentSplitId = nextSplit.splitId();
    }

    private /*RecordsWithSplitIds<RecordAndPosition>*/  FileRecords finisheSplit() throws IOException{
        if (currentReader != null) {
            currentReader.close();
            currentReader = null;
